...
/**
 * IterativDeck Document Type
 * 
 * This document type handles iterative deck creation, management, and presentation.
 * It includes structured slides for investor presentations, demos, and storytelling.
 */
export interface IterativDeckDocument extends BaseDocument {
  type: 'iterativ-deck';
  content: IterativDeckContent;
  metadata: IterativDeckMetadata;
}

export interface IterativDeckContent extends DocumentContent {
  format: 'structured';
  data: {
    slides: IterativDeckSlide[];
    theme: IterativDeckTheme;
...
export interface IterativDeckSlide {
  id: string;
  title: string;
  type: SlideType;
...
export interface IterativDeckTheme {
  name: string;
  colors: ThemeColors;
  fonts: ThemeFonts;
...
export interface IterativDeckMetadata extends DocumentMetadata {
...
// Factory function
export function createIterativDeck(data: Partial<IterativDeckDocument>): IterativDeckDocument {
  const now = new Date();
  ...
  return {
    ...
    content: data.content || createDefaultIterativDeckContent(),
    metadata: {
      category: 'pitch-deck',
...
function createDefaultIterativDeckContent(): IterativDeckContent {
  return {
    format: 'structured',
    data: {
      slides: [
