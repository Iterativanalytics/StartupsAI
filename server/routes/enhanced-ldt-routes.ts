...
import { LDTCollaborationService } from '../services/ldt-collaboration-service';
import { LDTAnalyticsService } from '../services/ldt-analytics-service';
import { LDTFacilitationAgent } from '../ai-agents/agents/design-thinking/ldt-facilitation-agent';
import { LDTInsightsAgent } from '../ai-agents/agents/design-thinking/ldt-insights-agent';
import { handleRouteError, requireAuth, requireResource } from '../utils/routeHelpers';

const router = express.Router();

// Initialize services
const collaborationService = new LDTCollaborationService();
const analyticsService = new LDTAnalyticsService();
const facilitationAgent = new LDTFacilitationAgent();
const insightsAgent = new LDTInsightsAgent();

// Enhanced LDT Workflow Routes
router.get('/workflows', authMiddleware, async (req, res) => {
  try {
    ...
    handleRouteError(error, res, 'Failed to fetch LDT workflows');
  }
});
...
router.post('/workflows', authMiddleware, async (req, res) => {
  try {
    ...
    handleRouteError(error, res, 'Failed to create LDT workflow');
  }
});
...
router.get('/workflows/:id', authMiddleware, async (req, res) => {
  try {
    ...
    handleRouteError(error, res, 'Failed to fetch LDT workflow');
  }
});
...
router.put('/workflows/:id', authMiddleware, async (req, res) => {
  try {
    ...
    handleRouteError(error, res, 'Failed to update LDT workflow');
  }
});
...
router.delete('/workflows/:id', authMiddleware, async (req, res) => {
  try {
    ...
    handleRouteError(error, res, 'Failed to delete LDT workflow');
  }
});
